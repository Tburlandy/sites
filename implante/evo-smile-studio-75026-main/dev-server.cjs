...
export const SITE_CONFIG = {
  ...
  horarioAtendimento: {
    ...
    sabado: "${escapeString(config.horarioAtendimento?.sabado || "Sábado: 8h às 12h")}", // Deixe vazio para não exibir
  },
...
`;
    ...
    // Salvar também o arquivo .env com webhookUrl, gtmId e Google Ads
    const envPath = path.join(__dirname, ".env");
    const envLines = [];
    
    if (config.webhookUrl && config.webhookUrl.trim() !== "") {
      envLines.push(`VITE_WEBHOOK_URL=${config.webhookUrl}`);
    }
    
    if (config.gtmId && config.gtmId.trim() !== "") {
      envLines.push(`VITE_GTM_ID=${config.gtmId}`);
    }
    
    if (config.googleAnalyticsId && config.googleAnalyticsId.trim() !== "") {
      envLines.push(`VITE_GOOGLE_ANALYTICS_ID=${config.googleAnalyticsId}`);
    }
    
    // Google Ads
    if (config.googleAds?.conversionId && config.googleAds.conversionId.trim() !== "") {
      envLines.push(`VITE_GOOGLE_ADS_CONVERSION_ID=${config.googleAds.conversionId}`);
    }
    
    if (config.googleAds?.conversionLabel && config.googleAds.conversionLabel.trim() !== "") {
      envLines.push(`VITE_GOOGLE_ADS_CONVERSION_LABEL=${config.googleAds.conversionLabel}`);
    }
    
    // Ler .env existente se houver e preservar outras variáveis
    ...
    // Parsear variáveis existentes que não estamos atualizando
    const existingLines = existingEnvContent.split("\n").filter(line => {
      const trimmed = line.trim();
      return trimmed && 
             !trimmed.startsWith("VITE_WEBHOOK_URL") && 
             !trimmed.startsWith("VITE_GTM_ID") &&
             !trimmed.startsWith("VITE_GOOGLE_ANALYTICS_ID") &&
             !trimmed.startsWith("VITE_GOOGLE_ADS_CONVERSION_ID") &&
             !trimmed.startsWith("VITE_GOOGLE_ADS_CONVERSION_LABEL") &&
             !trimmed.startsWith("#");
    });
    
    // Combinar novas variáveis com as existentes
    const finalEnvContent = [...envLines, ...existingLines].join("\n") + "\n";
    
    // Salvar .env
    await fs.writeFile(envPath, finalEnvContent, "utf-8");
    console.log("✅ Arquivo .env atualizado com sucesso");
    
    res.json({ success: true, message: "Configuração e .env salvos com sucesso" });
  } catch (error) {
