...
app.post("/api/media/upload", upload.fields([
  ...
  try {
    const { sectionKey, mediaType, description, transform } = req.body;
    ...
    // Parse transform se fornecido
    let parsedTransform = null;
    if (transform) {
      try {
        parsedTransform = typeof transform === 'string' ? JSON.parse(transform) : transform;
      } catch (e) {
        console.warn("Erro ao parsear transform:", e);
      }
    }
...
// PATCH /api/media/:sectionKey/:id - Atualizar transformação
app.patch("/api/media/:sectionKey/:id", async (req, res) => {
  try {
    const { sectionKey, id } = req.params;
    const { transform } = req.body;
    
    console.log(`[PATCH] Atualizando transformação para ${sectionKey}/${id}`, transform);
    
    const mediaSections = await readMediaSections();
    const section = mediaSections[sectionKey];
    
    if (!section) {
      console.error(`[PATCH] Seção não encontrada: ${sectionKey}`);
      return res.status(404).json({ error: "Seção não encontrada" });
    }
    
    const index = section.findIndex((item) => item.id === id);
    if (index === -1) {
      console.error(`[PATCH] Item não encontrado: ${id} na seção ${sectionKey}`);
      console.error(`[PATCH] IDs disponíveis:`, section.map(item => item.id));
      return res.status(404).json({ error: `Item não encontrado. ID: ${id}` });
    }
    
    // Atualizar transformação
    if (transform) {
      section[index].transform = transform;
    } else {
      delete section[index].transform;
    }
    
    await saveMediaSections(mediaSections);
    
    console.log(`[PATCH] Transformação salva com sucesso para ${sectionKey}/${id}`);
    res.json(section[index]);
  } catch (error) {
    console.error("Erro ao atualizar transformação:", error);
    res.status(500).json({ error: error.message });
  }
});

// Também aceitar PUT como alternativa (alguns clientes não suportam PATCH bem)
app.put("/api/media/:sectionKey/:id", async (req, res) => {
  try {
    const { sectionKey, id } = req.params;
    const { transform } = req.body;
    
    console.log(`[PUT] Atualizando transformação para ${sectionKey}/${id}`, transform);
    
    const mediaSections = await readMediaSections();
    const section = mediaSections[sectionKey];
    
    if (!section) {
      console.error(`[PUT] Seção não encontrada: ${sectionKey}`);
      return res.status(404).json({ error: "Seção não encontrada" });
    }
    
    const index = section.findIndex((item) => item.id === id);
    if (index === -1) {
      console.error(`[PUT] Item não encontrado: ${id} na seção ${sectionKey}`);
      console.error(`[PUT] IDs disponíveis:`, section.map(item => item.id));
      return res.status(404).json({ error: `Item não encontrado. ID: ${id}` });
    }
    ...
    console.log(`[PUT] Transformação salva com sucesso para ${sectionKey}/${id}`);
    res.json(section[index]);
  } catch (error) {
    console.error("Erro ao atualizar transformação:", error);
    res.status(500).json({ error: error.message });
...
`;
    ...
    // Atualizar vite.config.ts com o basePath
    try {
      const viteConfigPath = path.join(__dirname, "vite.config.ts");
      let viteConfigContent = await fs.readFile(viteConfigPath, "utf-8");
      
      // Substituir o base no vite.config.ts
      const basePath = config.basePath || "/pagina/";
      const baseRegex = /base:\s*['"`]([^'"`]+)['"`]/;
      
      if (baseRegex.test(viteConfigContent)) {
        viteConfigContent = viteConfigContent.replace(baseRegex, `base: '${basePath}'`);
      } else {
        // Se não encontrar, adicionar após o defineConfig
      ...
      console.log(`✅ vite.config.ts atualizado com base: '${basePath}'`);
    } catch (viteError) {
      console.warn("⚠️ Não foi possível atualizar vite.config.ts:", viteError.message);
    }
    
    res.json({ 
      success: true, 
      message: `Configuração salva com sucesso! Site URL: ${config.siteUrl}, Base Path: ${config.basePath || "/pagina/"}` 
    });
  } catch (error) {
