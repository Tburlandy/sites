...
export function ConfigEditor() {
  ...
  const [config, setConfig] = useState<ConfigState>({
    siteUrl: SITE_CONFIG.siteUrl || "https://topimplantes.com",
    basePath: "/pagina/",
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>URL e caminho base do site</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <label className="text-sm font-medium mb-2 block">URL do Site</label>
            <Input
              value={config.siteUrl}
              onChange={(e) => updateField("siteUrl", e.target.value)}
              placeholder="https://topimplantes.com"
              type="url"
            />
            <p className="text-xs text-muted-foreground mt-1">
              URL base do site (sem barra final). Usado para SEO e links canônicos.
            </p>
          </div>

          <div>
            <label className="text-sm font-medium mb-2 block">Caminho Base (Base Path)</label>
            <Input
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              Caminho base onde o site será instalado (ex: /pagina/). Deve começar e terminar com barra.
            </p>
            <p className="text-xs text-warning mt-1">
              ⚠️ Após alterar o Base Path, você precisa atualizar manualmente o arquivo vite.config.ts e fazer rebuild.
            </p>
          </div>
        </CardContent>
      </Card>
