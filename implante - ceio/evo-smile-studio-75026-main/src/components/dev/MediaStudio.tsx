                    ...
                    <CardContent className="pt-6 space-y-4">
                      {item.type === "image" ? (
                        <ZoomableImage
                          ...
                          itemId={item.id}
                          sectionKey={selectedSection}
                          initialZoom={item.zoom || 1}
                          initialPositionX={item.positionX || 0}
                          initialPositionY={item.positionY || 0}
                          onSave={async (zoom, positionX, positionY) => {
                            try {
                              const response = await fetch(`${API_BASE}/api/media/${selectedSection}/${item.id}/transform`, {
                                method: "PATCH",
                                headers: {
                                  "Content-Type": "application/json",
                                },
                                body: JSON.stringify({ zoom, positionX, positionY }),
                              });
                        ...
                        <div className="rounded-lg overflow-hidden border">
                          <video src={getAssetPath(item.src)} controls className="w-full" poster={item.poster ? getAssetPath(item.poster) : undefined} />
                        </div>
                      )}
